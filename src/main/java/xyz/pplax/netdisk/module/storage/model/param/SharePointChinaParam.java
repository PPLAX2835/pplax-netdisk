...
public class SharePointChinaParam extends SharePointParam {
	...
	@StorageParamItem(name = "clientId", defaultValue = "${pplax.onedrive-china.clientId}", order = 1)
	private String clientId;

	@StorageParamItem(name = "SecretKey", defaultValue = "${pplax.onedrive-china.clientSecret}", order = 2)
	private String clientSecret;
	
	@StorageParamItem(name = "回调地址", description = "如使用自定义 api, 需将此处默认的域名修改为您的域名, 且需在 api 中配置为回调域名.",
			defaultValue = "${pplax.onedrive-china.redirectUri}", order = 3)
